...
export default async function handler(
  ...
  if (!apiKey) {
    res.status(500).json({ error: 'Proxycurl API Key not configured on the server' });
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Proxycurl API error:', response.status, errText);
      res.status(response.status).json({ error: `Proxycurl error: ${response.statusText}` });
      return;
    }
    ...
    console.error('Error fetching from Proxycurl:', error);
