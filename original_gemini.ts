...
export async function extractNormalizedProfiles(
  ...
  for (let bIdx = 0; bIdx < batches.length; bIdx++) {
    ...
    const prompt = `Tu es un analyste d'intelligence commerciale (Business Intelligence). Pour chaque contact ci-dessous, extrais un profil orient├® BUSINESS et MON├ëTISATION.

${userContext}

Ton but : extraire les signaux COMMERCIAUX de chaque profil en pensant ├á comment L'UTILISATEUR CI-DESSUS pourrait en tirer profit (vente, partenariat, commission, influence, etc.).

Contacts ├á analyser :
${JSON.stringify(batchData, null, 2)}

Pour CHAQUE contact, extrais les informations suivantes. Si une donn├®e n'est pas disponible, D├ëDUIS-la intelligemment du poste, secteur et contexte. Sois PR├ëCIS et CONCRET, pas g├®n├®rique.
...
[
  {
    ...
    "name": "Nom complet",
    "sector": "Secteur d'activit├® normalis├® (ex: FinTech, EdTech, SaaS, Immobilier, Sant├®, Consulting, etc.)",
    "roleCategory": "D├®cideur | Technique | Commercial | Cr├®atif | Op├®rationnel | Support",
    "seniority": "Junior | Mid | Senior | C-Level | Fondateur",
    "explicitNeeds": ["Besoins EXPLICITEMENT mentionn├®s dans les notes ou bio ÔÇö sois tr├¿s sp├®cifique"],
    "inferredNeeds": ["Besoins D├ëDUITS du poste et secteur. Pense business : un CEO cherche des clients/investisseurs, un CTO cherche des devs, un commercial cherche des leads, un freelance cherche des missions"],
    "skillsOffered": ["Ce que cette personne PEUT VENDRE ou OFFRIR concr├¿tement (expertise pr├®cise, r├®seau dans un secteur, pouvoir de d├®cision, budget)"],
    "painPoints": ["Frustrations BUSINESS : manque de clients, co├╗t d'acquisition trop ├®lev├®, difficult├® ├á recruter, manque de financement, etc."],
    ...
    "networkValue": "connector | influencer | specialist | dormant",
    "buyingSignals": ["Signaux d'achat concrets : cherche un prestataire, recrute, l├¿ve des fonds, lance un projet, change de poste, croissance rapide"],
    "relationshipStrength": "cold | warm | hot | strategic (bas├® sur la quantit├® de notes/d├®tails disponibles : beaucoup d'info = hot/strategic, peu = cold/warm)",
    "whatUserCanDoForThem": "Une phrase CONCR├êTE d├®crivant ce que l'utilisateur pourrait apporter ├á cette personne (ex: 'Lui pr├®senter 3 investisseurs de son r├®seau', 'Lui fournir un dev React pour son projet')",
    "whatTheyCanDoForUser": "Une phrase CONCR├êTE d├®crivant ce que cette personne pourrait apporter ├á l'utilisateur (ex: 'Ouvrir la porte chez L'Or├®al', 'Devenir client pour du consulting growth')"
  }
]`;
...
/**
 * PASSE 2 ÔÇö Compute Embeddings for each profile
  ...
  for (let i = 0; i < profiles.length; i += batchSize) {
    ...
    const embedPromises = batch.map(async (profile) => {
      ...
      const parts = [
        ...
        profile.inferredNeeds.length > 0 ? `Besoins d├®duits : ${profile.inferredNeeds.join(', ')}.` : '',
        profile.painPoints.length > 0 ? `Probl├¿mes business : ${profile.painPoints.join(', ')}.` : '',
        ...
        profile.budgetAuthority && profile.budgetAuthority !== 'none' ? `Pouvoir de d├®cision : ${profile.budgetAuthority}.` : '',
        profile.networkValue ? `Valeur r├®seau : ${profile.networkValue}.` : '',
        profile.topicsOfInterest.length > 0 ? `Centres d'int├®r├¬t : ${profile.topicsOfInterest.join(', ')}.` : '',
...
/**
 * PASSE 3 ÔÇö Build Supply/Demand Matrix
...
export async function buildSupplyDemandAnalysis(
  ...
  // Build condensed data for the prompt ÔÇö include business signals
  ...
  const prompt = `Tu es un analyste en intelligence de r├®seau pour des professionnels de la mise en relation, du consulting, du freelance, de la lev├®e de fonds et de l'├®v├®nementiel.

Ton objectif : identifier les besoins MON├ëTISABLES dans ce r├®seau. Un besoin mon├®tisable = un probl├¿me pour lequel quelqu'un est PR├èT ├Ç PAYER (en argent, en temps, en acc├¿s).

Profils du r├®seau (avec signaux d'achat) :
...
Profil de l'utilisateur (propri├®taire du r├®seau) :
${JSON.stringify(userProfile, null, 2)}

INSTRUCTIONS :
1. Identifie les 10-15 besoins les plus CONCRETS et MON├ëTISABLES du r├®seau
2. Priorise les besoins o├╣ il y a des signaux d'achat (gens qui cherchent activement, qui ont le budget, qui sont en phase de d├®cision)
3. Pour chaque besoin, identifie les demandeurs ET les fournisseurs potentiels dans le r├®seau
4. ├ëvalue le gap : "covered" (offre suffisante), "partial" (quelques fournisseurs), "opportunity" (forte demande, pas d'offre ÔåÆ le user peut intervenir)
5. Indique si le user peut combler ce gap avec ses comp├®tences

EXEMPLES de besoins mon├®tisables :
- "Besoin de d├®veloppeurs React/Next.js pour un projet 6 mois" (pas juste "besoin tech")
- "Recherche d'investisseurs seed pour SaaS B2B (300-500KÔé¼)" (pas juste "financement")
- "Besoin d'un consultant growth pour atteindre 100 clients" (pas juste "croissance")
- "Recherche de partenaires commerciaux sur le march├® fran├ºais" (pas juste "partenariats")

Retourne UNIQUEMENT un tableau JSON :
[
  {
    "need": "Description PR├ëCISE et CHIFFR├ëE du besoin mon├®tisable",
...
Trie par potentiel de mon├®tisation : "opportunity" avec budget-holders d'abord.`;
...
/**
 * PASSE 3b ÔÇö Name clusters using Gemini
...
export async function nameClusters(
  ...
  const prompt = `Tu es un expert en analyse de communaut├®s. Voici des groupes de personnes regroup├®es AUTOMATIQUEMENT par proximit├® s├®mantique (via un algorithme K-means sur des embeddings vectoriels).

R├êGLES STRICTES :
1. CHAQUE PERSONNE N'APPARA├ÄT QUE DANS UN SEUL CLUSTER. Ne duplique JAMAIS un contact dans plusieurs clusters.
2. Les membres de chaque cluster sont CEUX QUI SONT LIST├ëS dans le cluster ÔÇö tu ne peux PAS les d├®placer.
3. Donne un nom accrocheur au cluster qui capture PR├ëCIS├ëMENT le point commun de ces personnes sp├®cifiques.
4. Les "commonNeeds" et "commonSkills" doivent ├¬tre des choses que la MAJORIT├ë des membres du cluster partagent r├®ellement, pas des g├®n├®ralit├®s.
5. Si un cluster contient des profils tr├¿s vari├®s, sois honn├¬te dans le th├¿me (ex: "Profils entrepreneuriaux diversifi├®s") plut├┤t que d'inventer un lien artificiel.

Clusters (groupements calcul├®s par K-means) :
${JSON.stringify(clustersData, null, 2)}

Retourne un tableau JSON avec cette structure exacte. GARDE EXACTEMENT les m├¬mes membres dans chaque cluster, ne modifie pas les groupements :
[
  {
    "clusterId": 0,
    "clusterName": "Nom accrocheur et PR├ëCIS du groupe",
    "theme": "Th├¿me principal en une phrase courte",
    "members": [{ "id": "ID ORIGINAL du contact", "name": "Nom", "role": "R├┤le", "company": "Entreprise" }],
    "commonNeeds": ["Besoin vraiment partag├® par la majorit├®"],
    "commonSkills": ["Comp├®tence vraiment commune"],
...
/**
 * PASSE 4 ÔÇö Deep User Opportunity Analysis
...
Ton but : trouver les DEALS CACH├ëS dans ce r├®seau. Pas des id├®es vagues ÔÇö des opportunit├®s concr├¿tes avec un mod├¿le de revenu clair, ADAPT├ëES AU PROFIL SP├ëCIFIQUE de l'utilisateur ci-dessus.

## PROFIL DE L'UTILISATEUR (celui qui poss├¿de le r├®seau)
${JSON.stringify(userProfile, null, 2)}

## CLUSTERS D├ëTECT├ëS DANS LE R├ëSEAU
...
## GAPS IDENTIFI├ëS (Besoins mon├®tisables non couverts)
${JSON.stringify(gaps, null, 2)}

## OPPORTUNIT├ëS SP├ëCIFIQUES POUR L'UTILISATEUR
${JSON.stringify(userOpportunities, null, 2)}

## PROFILS D├ëTAILL├ëS DES CONTACTS (avec signaux d'achat)
...
G├ëN├êRE jusqu'├á 8 opportunit├®s concr├¿tes. Pour chaque opportunit├®, pense comme un deal maker :
- QUI paie ? (le client, un sponsor, une commission ?)
- COMBIEN ? (estimation r├®aliste en euros)
- QUAND ? (imm├®diat, 1 mois, 3 mois)
- COMMENT tu te fais payer ? (prestation, commission d'apporteur d'affaires, abonnement, ticket d'entr├®e, equity)

R├®partis en 4 cat├®gories :
1. **"service"** : Missions de consulting, formations payantes, accompagnement strat├®gique, coaching
2. **"product"** : Produits num├®riques (SaaS, templates, guides premium, outils) pour un besoin r├®current
3. **"connection"** : Mise en relation ├á haute valeur. L'utilisateur joue le BROKER : il prend une commission ou renforce sa position en orchestrant le deal
4. **"event"** : D├«ners priv├®s, masterclasses, cercles de r├®flexion exclusifs avec un ticket d'entr├®e ou des sponsors
...
[
  {
    ...
    "targetCluster": "Nom du cluster cibl├®",
    "demandScore": 8,
    "feasibilityScore": 7,
    "relevantContacts": [
      { "id": "ID", "name": "Nom", "role": "Poste", "company": "Entreprise", "reason": "Prospect / Partenaire / Ambassadeur / Sponsor ÔÇö et pourquoi" }
    ],
    "actionPlan": ["Action 1 faisable CETTE SEMAINE", "Action 2 dans les 2 semaines", "Action 3 dans le mois"],
    "estimatedImpact": "Impact business concret (ex: 5 clients ├á 2KÔé¼, 3 partenariats qui ouvrent 50 leads)",
    "revenueModel": "Comment tu gagnes de l'argent (ex: commission 10% sur mise en relation, 3KÔé¼/jour de consulting, ticket 200Ôé¼/personne)",
    "estimatedRevenue": "Estimation r├®aliste (ex: 5K-15KÔé¼ sur 3 mois)",
    "timeToRevenue": "D├®lai estim├® (ex: 2 semaines, 1 mois, 3 mois)",
    "urgency": "immediate | short-term | medium-term"
  }
]

R├êGLES :
- Priorise les opportunit├®s "immediate" avec des budget-holders identifi├®s
- Sois CONCRET sur les montants ÔÇö mieux vaut une fourchette r├®aliste que pas de chiffre
- Chaque contact dans "relevantContacts" doit avoir une raison SP├ëCIFIQUE (pas juste "int├®ress├® par le sujet")
- L'action plan doit ├¬tre faisable par UNE personne sans budget initial`;
...
/**
 * MASTER ORCHESTRATOR ÔÇö Runs the full 4-pass pipeline
 */
export async function runOracleV3Pipeline(
  ...
  if (profiles.length === 0) {
    throw new Error("Aucun profil n'a pu ├¬tre extrait. V├®rifiez vos contacts.");
  }
  ...
  if (embeddings.length >= 4) {
    ...
    // Find optimal K ÔÇö enforce minimum granularity
    ...
    // Not enough contacts for clustering ÔÇö use Gemini directly
    onPassChange?.(3, 0);
...
/**
 * PASSE 4b ÔÇö Detect Strategic Pairwise Introductions
...
async function detectStrategicIntros(
  ...
  const prompt = `Tu es un expert en mise en relation strat├®gique. Voici des paires de contacts qui pourraient b├®n├®ficier d'une introduction.

${buildUserContext(userProfile)}

Paires candidates (tri├®es par score de compatibilit├®) :
...
Pour chaque paire, ├®value si l'introduction a du SENS BUSINESS. ├ëlimine les paires sans vraie valeur ajout├®e.
Pour les paires retenues, explique CONCR├êTEMENT :
- Pourquoi ils devraient se rencontrer
- Ce que A gagne
- Ce que B gagne
- Ce que l'UTILISATEUR gagne en orchestrant cette intro (commission, positionnement, r├®ciprocit├®)

Retourne UNIQUEMENT un tableau JSON :
[
  {
    "contactA": { "id": "", "name": "Nom A", "role": "R├┤le", "company": "Entreprise" },
    "contactB": { "id": "", "name": "Nom B", "role": "R├┤le", "company": "Entreprise" },
    "reason": "Phrase d'accroche pour l'introduction (ex: 'Marie a exactement le profil d'investisseur que Paul recherche pour sa s├®rie A')",
    "valueForA": "Ce que A gagne concr├¿tement",
    "valueForB": "Ce que B gagne concr├¿tement",
...
Retourne entre 3 et 6 introductions les plus strat├®giques.`;
...
/**
 * PASSE 5 ÔÇö Analyze Network Genome & Valuation
 */
async function analyzeNetworkGenome(
  ...
  const prompt = `Tu es un auditeur financier sp├®cialis├® dans la valorisation de capital social et de r├®seaux d'affaires.

${buildUserContext(userProfile)}

Voici un r├®sum├® du r├®seau de l'utilisateur :
- Taille : ${profiles.length} contacts qualifi├®s
- Opportunit├®s directes d├®tect├®es : ${opportunities.length}
- Introductions strat├®giques d├®tect├®es : ${keyIntros.length}

D├®tails des opportunit├®s :
${JSON.stringify(opportunities.map(o => ({ title: o.title, estimatedRevenue: o.estimatedRevenue, urgency: o.urgency })), null, 2)}

Ton objectif : G├®n├®rer l'ADN du r├®seau (Network Genome) et estimer sa valeur financi├¿re ANNUELLE potentielle.

Instructions pour la valorisation financi├¿re (valuationScore) :
- Additionne la valeur basse estim├®e de chaque opportunit├®. Si c'est "2k-5kÔé¼", compte 2000Ôé¼.
- Ajoute 500Ôé¼ de valeur pour chaque introduction strat├®gique.
- Applique un multiplicateur bas├® sur la densit├® de d├®cideurs (budget-holder/decision-maker) dans le r├®seau.
...
Retourne un objet JSON avec la structure exacte suivante :
{
  ...
  "valuationReasoning": "Explication courte du calcul. Ex: 'Bas├® sur 3 opportunit├®s qualifi├®es (25KÔé¼) et 4 intros strat├®giques (2KÔé¼), plus une prime pour votre densit├® de d├®cideurs Tech.'",
  "networkPersona": "Un titre accrocheur d├®finissant le style du r├®seau (ex: 'Le Hub B2B Parisien', 'L'Insider FinTech', 'Le Connecteur de Talents')",
  "topStrengths": [
    "Force 1 : Tr├¿s forte concentration de fondateurs SaaS en recherche de fonds",
    "Force 2 : Excellente couverture des d├®cideurs RH",
    "Force 3 : Capacit├® ├á connecter la tech et le marketing"
  ],
  "blindSpots": [
    "Angle mort 1 : Beaucoup de startups mais aucun investisseur VC pour les financer",
    "Angle mort 2 : Un r├®seau tr├¿s francilien, peu d'ouvertures ├á l'international"
  ]
}`;
...
/**
 * PASSE 6 ÔÇö Analyze Reciprocity Engine
 */
...
Voici des contacts et les notes prises par l'utilisateur ├á leur sujet.
Ton but est de d├®tecter les "D├®s├®quilibres de R├®ciprocit├®" (Reciprocity Imbalance) ÔÇö c'est-├á-dire les relations o├╣ l'une des parties a rendu un service significatif ├á l'autre, et o├╣ un retour d'ascenseur est attendu ou recommand├®.

Donn├®es des contacts :
${JSON.stringify(contactData, null, 2)}

Pour chaque contact, d├®termine s'il y a un d├®s├®quilibre clair bas├® sur les mots utilis├®s dans les notes (ex: "m'a pr├®sent├® ├á", "je l'ai aid├® sur", "redevable", "a fait une intro"). S'il n'y en a pas, ignore-le.
...
[
  {
    ...
    "status": "user_owes_them" (l'utilisateur leur doit un service) OU "they_owe_user" (ils doivent un service ├á l'utilisateur),
    "reason": "Phrase courte expliquant pourquoi (ex: 'Pierre vous a mis en relation avec le CTO de LVMH')",
    "recommendedAction": "Action concr├¿te recommand├®e (ex: 'Invitez-le ├á d├®jeuner', 'Envoyez-lui un prospect')"
  }
]`;
