...
// Mistral API helper for AIInput — goes through the authenticated server-side
// proxy (api/ai/mistral-chat.ts) so the Mistral key never reaches the client.
const callMistralForAIInput = async (prompt: string): Promise<string> => {
  ...
  if (!response.ok) {
    ...
    throw new Error(err.error || `Mistral proxy error ${response.status}`);
  }
...
Réponds uniquement avec le JSON.`;
      ...
      let text = await callMistralForAIInput(prompt);
